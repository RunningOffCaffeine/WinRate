...
class MLTuner(tk.Tk):
    def __init__(
        ...
        # Get the calculated height and set it as the minimum
        ...
        # Initial setup of debug panel based on startup state
    ...
    def _build_gui(self):
        ...
        self.confidence_scale = ttk.Scale(
            ...
            command=self._update_confidence_from_slider
        )
        ...
        # --- Build Debug Panel (initially hidden) ---
    ...
    def _update_confidence_from_entry(self, event):
        """Called when Enter is pressed in the confidence entry box."""
        try:
            val = float(self.var_confidence_entry.get())
            clamped_val = max(0.5, min(1.0, val)) # Clamp between 0.5 and 1.0
            self.var_confidence.set(clamped_val)
            self.var_confidence_entry.set(f"{clamped_val:.4f}")
            self.set_confidence_cb(clamped_val)
            self.focus() # Move focus away from the entry
        except ValueError:
            # If invalid text, revert to the slider's value
            self.var_confidence_entry.set(f"{self.var_confidence.get():.4f}")


    def _build_debug_panel_widgets(self, parent_container):
        """Creates the widgets for the debug panel but doesn't display them yet."""
        ...
        self.pred_list = tk.Text(self.DEBUG_PANEL, height=6, wrap=tk.WORD, state=tk.DISABLED, relief="solid", borderwidth=1)
        ...
        self.log_console = tk.Text(self.DEBUG_PANEL, wrap=tk.WORD, state=tk.DISABLED, relief="solid", borderwidth=1)
    ...
    def _apply_delay(self):
        """Applies the delay from the entry box."""
        try:
    ...
    def _toggle_text_skip_mode(self):
        """Informs the main script about the text skip state change."""
        self.set_text_skip_cb(self.var_text_skip.get())
    
    def _toggle_debug_panel_visibility(self):
        """Shows or hides the debug panel and adjusts window size."""
    ...
    def _refresh_debug_panel_data(self):
        """Periodically updates the debug panel if it's visible."""
        ...
        if predictions != self._last_predictions:
            ...
            if predictions:
                self.pred_list.insert(tk.END, "\n".join(predictions))
            else:
                 self.pred_list.insert(tk.END, "No confident detections...")
    ...
    def _toggle_bot_pause_state(self):
        """Toggles the bot's pause state and updates the GUI accordingly."""
        if not self.pause_event.is_set():
            self.pause_event.set()
            self.btn_pause.config(text="Resume Bot", bg="#dc3545")
            self.status_label.config(text="Status: Paused")
        else:
            self.pause_event.clear()
            self.btn_pause.config(text="Pause Bot", bg="#28a745")
